...
bool ReceiveNamedFile(int sock, const char *fileName, const char *filePath) {
    ...
    if(IsFile(recvBuffer)) {
        char fullPath[100]; // copy filename and add path
        snprintf(fullPath, sizeof(fullPath), "%s/%s", filePath, fileName);
        ProcessFile(recvBuffer, numBytesRcvd, fullPath);
        ...
        recvBuffer[numBytesRcvd] = '\0'; // Terminate the string
        printf("%s\n", recvBuffer); // Print the thank you message
        return false; // file not received
    }
}
...
int checkUser(const char *username, userInfo *users, int userCount, bool *userExists) {
    ...
    //user not found return userCount
    return userCount;
}
...
int CreateDirectoryIfNotExist(const char* path) {
    ...
    if (stat(path, &st) == -1) {
        ...
        if (mkdir(path, 0700) == -1) { // 0700 permissions give read, write, and execute to the owner
            perror("mkdir() error");
